	...
	$(".checkselect").change(function(){
		...
		if(v == 1){
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为审核中吗？")){
				$.post('/sys/as-keywords/updatekeyword',{'id':kid,'checkStatus':1},function(result){
					if("success" == result){
			...
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为审核通过吗？")){
				$.post('/sys/as-keywords/updatekeyword',{'id':kid,'checkStatus':2},function(result){
					if("success" == result){
			...
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为不通过吗？")){
				$.post('/sys/as-keywords/updatekeyword',{'id':kid,'checkStatus':3},function(result){
					if("success" == result){
			...
			if(confirm("您确定要对["+obj.attr("keyword")+"]进行续费操作吗？")){
				//window.location.href='/xufei.action?keywords.id='+kid;
					ymPrompt.win({message:'/xufei.action?id='+kid,width:600,height:400,title:'当前为【'+obj.attr("keyword")+'】进行续费操作',handler:callBack,iframe:true})
				}
		}else if(v == 5){
			if(confirm("您确定要修改["+obj.attr("keyword")+"]的使用状态为不使用吗？")){
				$.post('/sys/as-keywords/updatekeyword',{'id':kid,'isUse':0},function(result){
					if("success" == result){
			...
			if(confirm("您确定要修改["+obj.attr("keyword")+"]的使用状态为使用吗？")){
				$.post('/sys/as-keywords/updatekeyword',{'id':kid,'isUse':1},function(result){
					if("success" == result){
