$().ready(function(){
	...
	$("#submitkeyword").click(function(){
		...
		if(serviceType == 0){
			...
			humane.error("对不起，数据不完整，您不能进行续费操作。");
		}else{
			
			$.post('/sys/as-keywords/keywordsxufei',
					{p:$("#servicetype").val()+"-"+years,'id':kid},function(result){
				if("exception" == result){
					humane.error("对不起，您提交的关键词出现异常，没有成功，请重试。");
				}else if("nomoney" == result){
					humane.error("对不起，您当前的余额不能支付本次申请，请充值后再进行申请。");
				}else{
					humane.error("恭喜你，续费成功！");
...
function jisuan(){
	...
	if(serviceType == 0){
		...
		$.post('/sys/as-keywords/jisuan',{p:serviceType+"-"+years},function(result){
			if("exception" == result){
