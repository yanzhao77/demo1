$().ready(function(){
	...
	$("#addRoleSubmit").click(function(){
		...
		if(a_roleName == ''){
			...
			$.post("addRole",{'roleName':a_roleName,'isStart':a_isStart},function(result){
				if("success" == result){
					alert("添加成功！");
					window.location.href="/sys/as-role/showRole";
	...
	$("#modifyRoleSubmit").click(function(){
		...
		if(m_roleName == ''){
			...
			$.post("/sys/as-role/updateRole",{'id':m_roleId,'roleName':m_roleName,'isStart':m_isStart},function(result){
				if("success" == result){
					alert("修改成功！");
					window.location.href="/sys/as-role/showRole";
				}else if("repeat" == result){
