	...
	$("#caiwuok").click(function(){
		if(userid <= 0){
			...
			if(confirm("您确定要执行当前财务操作吗？")){
				$.post('/sys/as-accountdetail/opeaccount',
						{
					'userId':userid,
					'userName':$("#searchUerText").val(),
					'money':$("#zijin").val(),
					'detailType':$("#zijintype").val(),
...
function loadUsers(){
	$.post("/searchuser",{'userCode':$("#searchUerText").val()},function(result){
		//var json = result;
		var userList = "<ul>";
