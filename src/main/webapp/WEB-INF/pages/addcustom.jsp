...
<form id="cform" action="/addsavecustom.action" method="post">
        	 	...
        	 	<select id="selectcustomtype" name="custom.customType">
        	 		<option value="0" selected="selected">--请选择--</option>
	      				<c:if test="customTypeList != null">
	      					<c:forEach items="customTypeList">
	      						<option value="${configTypeValue }">${configTypeValue }</option>
	      					</c:forEach>
	      				</c:if>
			...
			<li>省/地区：<select id="selectprovince" name="custom.province">
        	 		<option value="0" selected="selected">--请选择--</option>
	      				<c:if test="provinceList != null">
	      					<c:forEach items="provinceList">
	      						<option value="${provinceID }">${provinceID }</option>
	      					</c:forEach>
	      				</c:if>
	      		</select>
			</li>
	...
	<a href="javascript:void(0);" id="addcontact">添加一个联系人</a>
          <c:choose>
          <c:when test="contactList == null"><input type="hidden" id="c_count" value="0"/></c:when>
          <c:otherwise><input type="hidden" id="c_count" value="${contactList.size() }"/></c:otherwise></c:choose></div>
	<div class="info3">
		<table>
            <thead>
            <tr>
